...
export default function ExamSeatingManagement() {
  ...
  const scheduleGroups = useMemo(() => {
    ...
    for (const schedule of matchingSchedules) {
      const key = `${schedule.class_id}:${schedule.section_id || 0}`;
      const current = grouped.get(key) || {
        key,
        className: schedule.class?.class_name || "Class",
      ...
      current.scheduleIds.push(String(schedule.id));
      if (schedule.subject?.name && !current.subjects.includes(schedule.subject.name)) {
        current.subjects.push(schedule.subject.name);
      }
  ...
  const toggleScheduleGroup = (scheduleIds, selected) => {
  ...
  return (
    <div className="container-fluid py-3 exam-seating-page">
      <style>{`
        ...
        .exam-seating-workflow-card { min-height: 455px; overflow: hidden; }
          ...
          {activePlan && (
            <div className="mt-3">
              ...
              <div className="row g-3 no-print exam-seating-workflow-row align-items-stretch">
                ...
                <div className="col-xl-4">
                  <div className="card shadow-sm h-100 d-flex flex-column exam-seating-workflow-card">
                    <div className="card-header d-flex justify-content-between align-items-center"><span className="fw-semibold">3. Scheduled classes</span>{scheduleGroups.length > 0 && <span className="badge text-bg-success">{scheduleGroups.length} group{scheduleGroups.length === 1 ? "" : "s"}</span>}</div>
                    <div className="card-body flex-grow-1 exam-seating-scroll-body">
                      {scheduleGroups.map((group) => {
                        const selectedCount = group.scheduleIds.filter((id) => selectedScheduleIds.includes(String(id))).length;
                        const isChecked = selectedCount === group.scheduleIds.length;
                        return <label key={group.key} className="d-flex align-items-start gap-2 border rounded p-2 mb-2"><input className="mt-1" type="checkbox" checked={isChecked} onChange={(e) => toggleScheduleGroup(group.scheduleIds, e.target.checked)} /><span><strong>{group.className} {group.sectionName}</strong><span className="d-block small text-muted">{group.subjects.length} subject{group.subjects.length === 1 ? "" : "s"} · {normalizeTime(group.startTime)}–{normalizeTime(group.endTime)}</span><span className="d-block small text-muted text-truncate" title={group.subjects.join(", ")}>{group.subjects.join(", ")}</span></span></label>;
                      })}
                    ...
                    <div className="card-footer"><button disabled={busy || activePlan.status !== "draft" || !(activePlan.rooms || []).length || !selectedScheduleIds.length} className="btn btn-primary w-100" onClick={autoAllocate}>{seats.length ? "Reallocate students" : "Auto-allocate students"}</button></div>
                  </div>
                </div>
