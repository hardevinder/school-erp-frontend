...
const MarksEntry = () => {
  const [filters, setFilters] = useState({ class_id: "", section_id: "", exam_id: "", subject_id: "" });
  ...
  const handleExamChange = (e) => {
    ...
    const selectedClass = classExamSubjects.find(c => c.class_id === parseInt(filters.class_id));
    const selectedExam = selectedClass?.exams.find(e => e.exam_id === parseInt(exam_id));
 ...
 const handleFilterChange = (e) => {
  ...
  const fetchMarksEntryData = async () => {
    ...
    try {
      ...
      const prefill = {};
      const preAttendance = {};
      (res.data.existingResults || []).forEach(r => {
        prefill[`${r.student_id}_${r.component_id}`] = r.marks_obtained;
        preAttendance[`${r.student_id}_${r.component_id}`] = r.attendance || "P";
      });
      setMarks(prefill);
  ...
  const max = components.find(c => c.component_id === component_id)?.max_marks || 100;
  ...
  setMarks((prev) => ({ ...prev, [key]: isNaN(num) ? "" : num }));
};


  const handleSaveMarks = async () => {
    if (!examScheduleId) return Swal.fire("Error", "Exam Schedule ID not found", "error");
    ...
    students.forEach((student) => {
      components.forEach((comp) => {
        ...
        marksData.push({
          ...
          marks_obtained: value === "" ? null : parseFloat(value),
          attendance: att,
        });
      });
    });
  ...
  const handleImportExcel = async (e) => {
    ...
    if (!file || !examScheduleId) return Swal.fire("Error", "Please select a file and valid schedule", "error");
    ...
    try {
      ...
      Swal.fire("Success", "Marks imported successfully", "success");
      fetchMarksEntryData();
      e.target.value = null; // ✅ Clear the file input after import
    } catch (err) {
...
const handleLockComponent = async (exam_scheme_id) => {
  ...
  const selectedClass = classExamSubjects.find(c => c.class_id === parseInt(filters.class_id));
  const selectedSection = sections.find(s => s.id === parseInt(filters.section_id));
  const selectedExam = exams.find(e => e.exam_id === parseInt(filters.exam_id));
  const isExamLocked = selectedExam?.is_locked;
  const selectedSubject = subjects.find(s => s.id === parseInt(filters.subject_id));
    ...
    <div className="container mt-4">
        ...
        <div className="card-body">
          ...
          <div className="row">
            <div className="col-md-3 mb-3">
              <label>Class</label>
              <select className="form-control" value={filters.class_id} onChange={handleClassChange}>
            ...
            <div className="col-md-3 mb-3">
              <label>Section</label>
              <select className="form-control" name="section_id" value={filters.section_id} onChange={handleFilterChange}>
            ...
            <div className="col-md-3 mb-3">
              <label>Subject</label>
              <select className="form-control" name="subject_id" value={filters.subject_id} onChange={handleFilterChange}>
      ...
      {selectedClass && selectedSection && selectedExam && selectedSubject && (
        <div className="alert alert-info">
          <strong>Selected Details:</strong><br />
          ...
          <div className="card-body">
            ...
            <div className="d-flex gap-3 mb-3">
              <button
                  ...
                  disabled={isExamLocked || students.length === 0 || components.length === 0}
                >
                  💾 Save Marks
                </button>

                <button className="btn btn-outline-primary" onClick={handleExportExcel} disabled={isExamLocked}>
                  ⬇️ Export Excel
                </button>
              ...
              <table className="table table-bordered table-striped">
                ...
                <tbody>
                  {students.map(student => (
                    <tr key={student.id} className={activeStudentId === student.id ? "table-primary" : ""}>
                      ...
                      {components.map(comp => {
                        ...
                        const value = marks[key] || "";
                        const att = attendance[key] || "P";
                        const isInvalid = parseFloat(value) > comp.max_marks;
                        ...
                        return (
                         ...
                         <td className={comp.is_locked ? "bg-light text-muted" : ""}>
                             <select
                                ...
                                onChange={(e) => handleAttendanceChange(student.id, comp.component_id, e.target.value)}
                              >

                                {["P", "A", "L", "ACT", "LA", "ML", "X"].map((opt) => (
                            ...
                            <td className={comp.is_locked ? "bg-light text-muted" : ""}>
                              <input
                                ...
                                value={value}
                                onChange={(e) => handleMarksChange(student.id, comp.component_id, e.target.value)}
                                disabled={att !== "P" || comp.is_locked}
                                  ...
                                  onKeyDown={(e) => {
                                    ...
                                    const currentCompIndex = components.findIndex(c => c.component_id === comp.component_id);
                                    let nextKey = null;
                              ...
                              {isInvalid && <div className="invalid-feedback">Max: {comp.max_marks}</div>}
                            </td>
                          </React.Fragment>
                        );
                      })}
