...
export default function StudentLeadershipCouncil() {
  ...
  return <div className="leadership-page">
    ...
    <div className="leadership-toolbar"><select value={sessionId} onChange={(e) => setSessionId(e.target.value)}>{meta.sessions.map((s) => <option key={s.id} value={s.id}>{s.name}{s.is_active ? " (Active)" : ""}</option>)}</select><button className="btn-lead primary" disabled={!sessionId} onClick={() => run(() => openCouncilPdf(sessionId))}>🖨 Full Branded Council PDF</button><span className="mini-note">{activeAppointments.length} active leadership appointment(s) • {activeSession?.name || "Session"}</span></div>
