...
export default function LibraryManagement({ mode = "dashboard", studentView = false }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (studentView) {
        const res = await api.get("/StudentsApp/me/library");
        setIssues(res.data?.issues || []);
        setDashboard({
          totals: {
            books: res.data?.active?.length || 0,
            copies: res.data?.issues?.length || 0,
            availableCopies: 0,
            issuedCopies: res.data?.active?.length || 0,
            overdueIssues: (res.data?.active || []).filter((x) => x.is_overdue).length,
            lostCopies: (res.data?.issues || []).filter((x) => x.status === "lost").length,
          },
        });
        return;
      }
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <div className="row g-3 mb-3">
        {[
          ["Books", totals.books || 0, "bi-journal-bookmark"],
          ["Total Copies", totals.copies || 0, "bi-collection"],
          ["Available", totals.availableCopies || 0, "bi-check2-circle"],
          ["Issued", totals.issuedCopies || 0, "bi-box-arrow-up-right"],
            ...
            <div className="border rounded bg-white p-3">
              ...
              <form onSubmit={issueBook} className="row g-2">
                ...
                <div className="col-md-6">
                  ...
                  <select className="form-select form-select-sm" value={issueForm.borrower_type} onChange={(e) => setIssueForm((x) => ({ ...x, borrower_type: e.target.value }))}>
