...
const buildPublicAssetURL = (value) => {
  ...
  return `${assetBase}/${encodeAssetPath(clean)}`;
};
...
const imageUrlToDataURL = async (url) => {
  ...
  try {
    ...
    console.warn("Student photo could not be embedded as data URL:", url, error?.message || error);
    return url;
  }
};
...
const FinalResultSummary = () => {
  ...
  const getReportCardSchoolLogoUrl = (formatAssets = {}) =>
    formatAssets.school_logo_url || buildPublicAssetURL(getReportCardFormatValue("school_logo_url"));

  const getReportCardBoardLogoUrl = (formatAssets = {}) =>
    formatAssets.board_logo_url || buildPublicAssetURL(getReportCardFormatValue("board_logo_url"));

  const prepareReportFormatAssetsForPdf = async () => {
    const schoolLogoUrl = buildPublicAssetURL(getReportCardFormatValue("school_logo_url"));
    const boardLogoUrl = buildPublicAssetURL(getReportCardFormatValue("board_logo_url"));

    const [schoolLogoDataUrl, boardLogoDataUrl] = await Promise.all([
      schoolLogoUrl ? imageUrlToDataURL(schoolLogoUrl) : Promise.resolve(""),
      boardLogoUrl ? imageUrlToDataURL(boardLogoUrl) : Promise.resolve(""),
    ]);

    return {
      school_logo_url: schoolLogoDataUrl || schoolLogoUrl || "",
      board_logo_url: boardLogoDataUrl || boardLogoUrl || "",
    };
  };
  ...
  const loadReportFormatForClass = async (class_id, session_id) => {
    ...
    try {
      ...
      const res = await api.get("/report-card-formats/format-by-class", { params });
      const format = normalizeReportFormatResponse(res.data);

      console.log("✅ FORMAT-BY-CLASS RAW RESPONSE:", res.data);
      console.log("✅ NORMALIZED REPORT FORMAT:", format);

      setReportFormat(format);
      return format;
    } catch (error) {
      console.warn("Report card format not found. Using selected template instead:", error);
      setReportFormat(null);
      return null;
    }
  };
 ...
 const getSubjectTermCompDisplay = (student, subjectName, termId, componentId) => {
  ...
  const buildSmartTemplateData = (student, infoOverride = {}) => {
    ...
    // SMCIS_DYNAMIC_CO_SCHOLASTIC_ROWS_V3
    // SMCIS Dynamic Report Card reads Part-2 from `co_scholastic.rows`.
    // Build that array from BOTH normal Co-Scholastic Area evaluations and
    // Subject.type = Co-Scholastic grade-only subjects (GK, Value Education, etc.).
    const dynamicCoRowsV3Map = new Map();
    const dynamicCoKeyV3 = (value) => String(value || "").trim().toLowerCase();
    const dynamicHasGradeV3 = (value) => {
      const grade = String(value ?? "").trim();
      return Boolean(grade && grade !== "-");
    };

    const dynamicPutCoRowV3 = (name, grade, serialOrder = 0, source = "area") => {
      const cleanName = String(name || "").trim();
      if (!cleanName) return;
      const key = dynamicCoKeyV3(cleanName);
      const existing = dynamicCoRowsV3Map.get(key);
      const nextGrade = dynamicHasGradeV3(grade) ? String(grade).trim() : "-";
      if (!existing) {
        dynamicCoRowsV3Map.set(key, {
          name: cleanName,
          grade: nextGrade,
          serial_order: Number(serialOrder || 0),
          source,
      ...
      // Prefer a real grade over a blank/dash value, while preserving the
      // existing mapped-area order/name if the same item exists in both sources.
      if (!dynamicHasGradeV3(existing.grade) && dynamicHasGradeV3(nextGrade)) {
        existing.grade = nextGrade;
      }
    };

    // Existing Co-Scholastic Area module rows (Drawing, Work Education, etc.).
    // For a one-term report use that term. If two terms are selected, the later
    // selected term is the displayed Grade column for this SMCIS template.
    const dynamicPreferredTermV3 = term2Id || term1Id || null;
    if (dynamicPreferredTermV3) {
      const bucket = coScholasticByTerm[String(dynamicPreferredTermV3)] || {};
      const rows = Object.values(bucket[String(student.id)] || {});
    ...
    // Subject master rows marked Co-Scholastic.
    (coSubjectNames || []).forEach((subjectName, index) => {
      const subjectRows = (student?.components || []).filter(
        (component) =>
          String(component?.subject_name || component?.subject || "").trim() ===
            String(subjectName || "").trim() &&
          isCoScholasticComponent(component)
      );

      const subjectId = subjectRows
      ...
      const directGrade = subjectRows
        .map((row) => row?.grade ?? row?.grade_obtained ?? row?.grade_value)
        .find(dynamicHasGradeV3);
      const subjectGrade = subjectId != null
        ? student?.subject_grades?.[subjectId] ?? student?.subject_grades?.[String(subjectId)]
        : null;

      const preferredGrade = term2Id
        ? [t2Grade, t1Grade, directGrade, subjectGrade].find(dynamicHasGradeV3)
        : [t1Grade, t2Grade, directGrade, subjectGrade].find(dynamicHasGradeV3);

      dynamicPutCoRowV3(subjectName, preferredGrade || "-", 10000 + index, "subject");
    });

    // Keep Drawing fallback used by older SMCIS data where Drawing was not yet
    // configured as Subject.type = Co-Scholastic.
    if (!(coSubjectNames || []).some(isDrawingSubject)) {
      const drawingGrade = term2Id
        ? [drawingT2, drawingT1].find(dynamicHasGradeV3)
        : [drawingT1, drawingT2].find(dynamicHasGradeV3);
      if (dynamicHasGradeV3(drawingGrade)) {
        dynamicPutCoRowV3("Drawing", drawingGrade, 9999, "drawing");
      }
    }

    const dynamicCoScholasticRowsV3 = Array.from(dynamicCoRowsV3Map.values())
      .sort((a, b) => Number(a.serial_order || 0) - Number(b.serial_order || 0))
    ...
    return {
      ...
      co_scholastic: { rows: dynamicCoScholasticRowsV3 },
    };
  };
...
`
  ...
  const getAllSubjectsForPrimary = (student) =>
    Array.from(new Set((student?.components || []).map((c) => c.subject_name).filter(Boolean)));
  ...
  const getPrimarySubjectComponents = (student, subjectName) => {
    ...
    (student?.components || [])
      .filter((c) => c.subject_name === subjectName)
      .forEach((c) => {
  ...
  const buildPrimarySubjectTableHtml = (student, subjectName) => {
    ...
    return `
      <table class="primary-subject-table">
        <thead>
          <tr>
            <th colspan="3" class="primary-subject-title">${escapePrimaryHtml(subjectName)}</th>
          </tr>
          <tr>
            <th class="skill-name-col">Assessment / Component</th>
            <th>${term1Id ? termLabel(term1Id) : "Term-I"}</th>
            <th>${term2Id ? termLabel(term2Id) : "Term-II"}</th>
          </tr>
        </thead>
        <tbody>
          ${
                  ...
                  .map((component) => {
                    ...
                    return `
                      <tr>
                        ...
                        <td>${escapePrimaryHtml(t1)}</td>
                        <td>${escapePrimaryHtml(t2)}</td>
                      </tr>
                    `;
                  })
                  .join("")
              : `<tr><td colspan="3">No components available</td></tr>`
          }
          <tr class="primary-overall-row">
            <td>Overall Grade</td>
            <td>${escapePrimaryHtml(s1?.grade || "-")}</td>
            <td>${escapePrimaryHtml(s2?.grade || "-")}</td>
          </tr>
        </tbody>
      </table>
    `;
  ...
  const buildPrimarySectionCardsHtml = (studentsForPdf = reportData || [], infoMapOverride = studentInfoMap, formatAssets = {}) => {
    const styles = `
      <style>
        * { box-sizing: border-box; }
        ...
        .primary-header {
          display: grid;
          grid-template-columns: 70px 1fr 58px 62px;
          gap: 5px;
          align-items: center;
        ...
        .box-body { padding: 3px 4px; line-height: 1.15; }
        .sign-row {
          display: grid;
          grid-template-columns: repeat(3, 1fr);
          gap: 16px;
    ...
    const cards = (studentsForPdf || []).map((student) => {
      ...
      const effectiveBoardLogoUrl = getReportCardBoardLogoUrl(formatAssets);
        ...
        <section class="primary-card">
          <div class="primary-header">
            <div>
              ${
                effectiveSchoolLogoUrl
                  ? `<img src="${escapePrimaryHtml(effectiveSchoolLogoUrl)}" class="primary-logo" />`
            ...
            <div style="display:flex;justify-content:center;align-items:center;">
              ${
                effectiveBoardLogoUrl
                  ? `<img src="${escapePrimaryHtml(effectiveBoardLogoUrl)}" alt="Board Logo" class="primary-board-logo" />`
                  : `<span style="display:block;width:50px;height:50px;"></span>`
              }
            </div>
          ...
          <div class="bottom-grid">
            ...
            <div class="small-box">
              ...
              <div class="box-body">
                <div><b>Height:</b> ${escapePrimaryHtml(info?.height || "-")} &nbsp; <b>Weight:</b> ${escapePrimaryHtml(info?.weight || "-")}</div>
                <div><b>Dental:</b> ${escapePrimaryHtml(info?.dental || "-")} &nbsp; <b>Vision:</b> ${escapePrimaryHtml(info?.vision || "-")} &nbsp; <b>Blood Group:</b> ${escapePrimaryHtml(info?.blood_group || "-")}</div>
              </div>
            </div>
            <div class="small-box">
        ...
        <div style="margin-bottom:8px">
          <div class="header-flex">
            <div class="header-left">
              ${
                effectiveSchoolLogoUrl
                  ? `<img src="${effectiveSchoolLogoUrl}" alt="School Logo" class="header-logo" />`
            ...
            <div class="header-right">
              ${
                effectiveBoardLogoUrl
                  ? `<img src="${escapePrimaryHtml(effectiveBoardLogoUrl)}" alt="Board Logo" class="header-board-logo" />`
                  : ""
              }
        ...
        {!loading && reportData.length > 0 && (
          <div className="mt-4">
            ...
            {reportData.map((student) => {
              ...
              const effectiveBoardLogoUrl = getReportCardBoardLogoUrl();
