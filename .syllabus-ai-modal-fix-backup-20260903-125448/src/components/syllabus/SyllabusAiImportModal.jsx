...
export default function SyllabusAiImportModal({ context, onClose, onUseDraft }) {
  ...
  const chooseFile = (selected) => {
    ...
    if (!allowedFile(selected)) {
      Swal.fire("Unsupported file", "Please choose a PDF, JPG, PNG or WEBP syllabus file.", "warning");
      return;
    }
    if (selected.size > 25 * 1024 * 1024) {
      Swal.fire("File too large", "Maximum syllabus file size is 25 MB.", "warning");
      return;
    }
    setFile(selected);
    setDraft(null);
  ...
  const analyze = async () => {
    if (!file) return Swal.fire("Choose a file", "Upload a syllabus PDF or handwritten image first.", "info");
    setAnalyzing(true);
    try {
      ...
      Swal.fire(
        "AI analysis failed",
        error?.response?.data?.message || "The syllabus could not be analyzed. Try a clearer scan or PDF.",
        "error"
      );
    } finally {
      setAnalyzing(false);
  ...
  const useDraft = () => {
    ...
    if (!units.length) {
      return Swal.fire("No units", "No syllabus units are available to use.", "warning");
    }
    if (invalid.length) {
      return Swal.fire("Complete highlighted rows", "Every unit/chapter needs a title before using this draft.", "warning");
    }
    onUseDraft?.(draft);
  };
