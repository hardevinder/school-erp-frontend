...
const ACTIONS = [
  ...
  { key: "library", label: "Learning Resources", description: "Open your digital resource library", icon: "bi-collection-play", path: "/my-library", roles: ["admin", "superadmin", "principal", "academic_coordinator", "coordinator", "teacher", "department_hod", "student"] },
];
...
export default function LmsWorkspaceDashboard({ role = "" }) {
  ...
  const actions = useMemo(() => {
    return ACTIONS.filter((item) => item.roles.includes(normalizedRole));
  }, [normalizedRole]);
