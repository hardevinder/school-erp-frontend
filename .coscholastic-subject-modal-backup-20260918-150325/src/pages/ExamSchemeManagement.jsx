...
const ExamSchemeManagement = () => {
  ...
  const [subjectType, setSubjectType] = useState("Scholastic");
  ...
  const fetchEvaluationMode = async () => {
    ...
    try {
      ...
      setSubjectType(res?.data?.subject_type || "Scholastic");
    } catch (e) {
      setEvaluationMode(null);
  ...
  const saveEvaluationMode = async () => {
    ...
    try {
      ...
      Swal.fire(
        "Saved",
        `${evaluationMode === "GRADE" ? "Grade" : "Marks"} Entry saved. ${subjectType === "Co-Scholastic" ? "This subject will appear first in report card co-scholastics with only its final grade." : "This subject will appear in scholastics."}`,
        "success"
      );
    } catch (e) {
      ...
      <div className="card mb-3 border-primary">
        <div className="card-body py-3">
          <div className="d-flex align-items-end justify-content-between flex-wrap gap-3">
            <div>
              <div className="fw-semibold">Marks Entry & Report Card</div>
              <div className="small text-muted">
            ...
            <div className="d-flex align-items-end gap-2 flex-wrap">
              <div>
                <label className="form-label small mb-1">Report card section</label>
                <select className="form-select" value={subjectType}
                  onChange={(event) => setSubjectType(event.target.value)}
                  disabled={!hasExactSchemeContext || !evaluationMode || evaluationModeLoading || evaluationModeSaving}>
                  <option value="Scholastic">Scholastic</option>
                  <option value="Co-Scholastic">Co-Scholastic (grade only)</option>
                </select>
                <div className="small text-muted">Subject classification applies across all classes and sessions.</div>
                <div className="small text-muted">With Marks Entry, the grade is calculated from weighted marks.</div>
              </div>
              <div>
                ...
                <select
                  ...
                  disabled={!hasExactSchemeContext || evaluationModeLoading || evaluationModeSaving}
                >
              ...
              <Button
                ...
                disabled={!hasExactSchemeContext || !evaluationMode || evaluationModeLoading || evaluationModeSaving}
              >
                {evaluationModeSaving ? "Saving..." : "Save Settings"}
              </Button>
            </div>
          </div>
        </div>
