...
const elementHtml = (el) => {
  const common = `data-rc-element="true" data-rc-id="${escapeHtml(el.id)}" data-type="${escapeHtml(el.type || "text")}" data-label="${escapeHtml(el.label || "Element")}" data-page="${Number(el.page || 1)}"`;
  if (el.type === "table") {
    return `<smart-marks-table ${common} data-source="${escapeHtml(el.table_source || "smart_table.rows")}" data-columns="${escapeHtml(JSON.stringify(el.columns || defaultColumns()))}" data-na-mode="${escapeHtml(el.na_mode || "shade")}" data-na-text="${escapeHtml(el.na_text || "N/A")}" data-na-logo-path="${escapeHtml(el.na_logo_path || "school.logo_url")}"></smart-marks-table>`;
  }
  if (el.type === "chart") {
    return `<subject-growth-chart ${common} data-source="${escapeHtml(el.chart_source || "smart_table.rows")}" data-category-source="${escapeHtml(el.category_source || "name")}" data-value-source="${escapeHtml(el.value_source || "percentage")}" data-title="${escapeHtml(el.chart_title || "SUBJECT WISE GROWTH")}" data-min="${Number(el.min_value ?? 0)}" data-max="${Number(el.max_value ?? 100)}" data-orientation="${escapeHtml(el.chart_orientation || "vertical")}" data-show-values="${el.show_values !== false}" data-show-grid="${el.show_grid !== false}" data-palette="${escapeHtml(JSON.stringify(el.chart_palette || []))}"></subject-growth-chart>`;
  }
  if (el.type === "image") {
    return `<img ${common} data-path="${escapeHtml(el.data_path || "")}" data-fallback="${escapeHtml(el.fallback ?? "-")}" alt="${escapeHtml(el.label || "Image")}" />`;
  }
  if (el.type === "box") return `<div ${common}></div>`;
  if (el.type === "line") return `<div ${common}></div>`;
  const dynamic = el.data_path ? `{{${el.data_path}}}` : "";
  const text = el.static_text || `${el.prefix || ""}${dynamic}${el.suffix || ""}` || el.label || "Text";
  return `<div ${common} data-path="${escapeHtml(el.data_path || "")}" data-prefix="${escapeHtml(el.prefix || "")}" data-suffix="${escapeHtml(el.suffix || "")}" data-static-text="${escapeHtml(el.static_text || "")}" data-fallback="${escapeHtml(el.fallback ?? "-")}">${escapeHtml(text)}</div>`;
};
...
const designToHtml = (draft) => {
  ...
  const pages = Array.from({ length: pageCount }, (_, index) => {
    ...
    return `  <section class="report-card-page ${escapeHtml(draft?.orientation || "portrait")}" data-page="${page}" data-orientation="${escapeHtml(draft?.orientation || "portrait")}">\n${children}\n  </section>`;
  }).join("\n");
  return `<!-- EduBridge Report Card editable HTML\n     Smart tags such as <smart-marks-table> and <subject-growth-chart> stay connected to ERP data.\n     Use data-path=\"student.name\" or {{student.name}} for dynamic fields. -->\n<div class="report-card-document" data-orientation="${escapeHtml(draft?.orientation || "portrait")}">\n${pages}\n</div>`;
};

const designToCss = (draft) => {
  const layout = Array.isArray(draft?.layout_json) ? draft.layout_json : [];
  const orientation = String(draft?.orientation || "portrait");
  const base = `/* EduBridge visual layout CSS. Edit supported properties and click Apply to Design. */\n.report-card-document { font-family: Arial, sans-serif; }\n.report-card-page { position: relative; width: ${orientation === "landscape" ? "297mm" : "210mm"}; height: ${orientation === "landscape" ? "210mm" : "297mm"}; background: #ffffff; overflow: hidden; }\n.rc-element, [data-rc-element=\"true\"] { position: absolute; box-sizing: border-box; }`;
  const rules = layout.map((el) => {
    const declarations = [
    ...
    if (el.type === "text") {
      declarations.push(`font-size: ${Number(el.font_size || 10)}pt`, `font-weight: ${el.bold ? 700 : 400}`, `text-align: ${el.align || "left"}`, `color: ${el.color || "#172033"}`, `background-color: ${el.background || "#ffffff"}`, `--rc-background-opacity: ${Number(el.background_opacity || 0)}`);
    } else if (el.type === "box") {
      declarations.push(`background-color: ${el.background || "#ffffff"}`, `--rc-background-opacity: ${Number(el.background_opacity ?? 1)}`, `border-color: ${el.border_color || "#cbd5e1"}`, `border-width: ${Number(el.border_width || 1)}px`, `border-style: solid`);
    } else if (el.type === "line") {
      declarations.push(`background-color: ${el.line_color || "#475569"}`, `--rc-line-width: ${Number(el.line_width || 1)}px`);
    } else if (el.type === "table") {
      declarations.push(`--rc-header-bg: ${el.header_bg || "#f1f5f9"}`, `--rc-header-color: ${el.header_color || "#172033"}`, `--rc-table-border: ${el.table_border_color || "#94a3b8"}`, `--rc-na-bg: ${el.na_bg || "#f8fafc"}`, `--rc-na-color: ${el.na_color || "#94a3b8"}`, `--rc-row-font-size: ${Number(el.row_font_size || 8.5)}pt`, `--rc-na-logo-opacity: ${Number(el.na_logo_opacity ?? 0.07)}`);
    } else if (el.type === "chart") {
      declarations.push(`--rc-title-color: ${el.title_color || "#164e63"}`, `--rc-grid-color: ${el.grid_color || "#e2e8f0"}`, `--rc-axis-color: ${el.axis_color || "#64748b"}`);
    }
    return `[data-rc-id="${String(el.id).replace(/"/g, "")}"] {\n  ${declarations.join(";\n  ")};\n}`;
  }).join("\n\n");
  return `${base}\n\n${rules}\n`;
};

const parseCssRuleMap = (cssText = "") => {
  const clean = String(cssText).replace(/\/\*[\s\S]*?\*\//g, "");
  const map = {};
  const re = /\[data-rc-id\s*=\s*["']([^"']+)["']\]\s*\{([\s\S]*?)\}/gi;
  let match;
  while ((match = re.exec(clean))) {
    ...
    map[match[1]] = declarations;
  }
  return map;
};

const cssNumber = (rules, key, fallback) => {
  const raw = rules?.[key];
  if (raw == null || raw === "") return Number(fallback || 0);
  const value = Number.parseFloat(String(raw));
  return Number.isFinite(value) ? value : Number(fallback || 0);
};
const cssColor = (rules, key, fallback) => {
  const value = String(rules?.[key] || "").trim();
...
const codeToDesign = (htmlText, cssText, currentDraft) => {
  const rawHtml = String(htmlText || "");
  if (/<\s*(script|iframe|object|embed)\b/i.test(rawHtml) || /\son\w+\s*=/i.test(rawHtml) || /javascript\s*:/i.test(rawHtml)) {
    throw new Error("Scripts, iframes, event handlers and javascript: URLs are not allowed in report-card HTML.");
  }
  ...
  const cssMap = parseCssRuleMap(cssText);
  ...
  if (!nodes.length) throw new Error("No report-card elements were found. Keep data-rc-element=\"true\" or use the smart component tags.");
  const layout = nodes.map((node) => {
    ...
    const next = {
      ...
      x_pct: clamp(cssNumber(rules, "left", base.x_pct), 0, 100),
      y_pct: clamp(cssNumber(rules, "top", base.y_pct), 0, 100),
      w_pct: clamp(cssNumber(rules, "width", base.w_pct), 1, 100),
      h_pct: clamp(cssNumber(rules, "height", base.h_pct), 1, 100),
      opacity: clamp(cssNumber(rules, "opacity", base.opacity ?? 1), 0.05, 1),
    };
    if (type === "text") {
      ...
      next.font_size = cssNumber(rules, "font-size", base.font_size || 10);
      next.bold = cssNumber(rules, "font-weight", base.bold ? 700 : 400) >= 600;
      next.align = ["left", "center", "right"].includes(String(rules["text-align"] || "").toLowerCase()) ? String(rules["text-align"]).toLowerCase() : base.align || "left";
      next.color = cssColor(rules, "color", base.color || "#172033");
      next.background = cssColor(rules, "background-color", base.background || "#ffffff");
      next.background_opacity = clamp(cssNumber(rules, "--rc-background-opacity", base.background_opacity || 0), 0, 1);
    } else if (type === "image") {
      next.data_path = node.getAttribute("data-path") || base.data_path || "";
      next.fallback = node.getAttribute("data-fallback") ?? base.fallback ?? "-";
    } else if (type === "box") {
      next.background = cssColor(rules, "background-color", base.background || "#ffffff");
      next.background_opacity = clamp(cssNumber(rules, "--rc-background-opacity", base.background_opacity ?? 1), 0, 1);
      next.border_color = cssColor(rules, "border-color", base.border_color || "#cbd5e1");
      next.border_width = cssNumber(rules, "border-width", base.border_width || 1);
    } else if (type === "line") {
      next.line_color = cssColor(rules, "background-color", base.line_color || "#475569");
      next.line_width = cssNumber(rules, "--rc-line-width", base.line_width || 1);
    } else if (type === "table") {
      ...
      next.header_bg = cssColor(rules, "--rc-header-bg", base.header_bg || "#f1f5f9");
      next.header_color = cssColor(rules, "--rc-header-color", base.header_color || "#172033");
      next.table_border_color = cssColor(rules, "--rc-table-border", base.table_border_color || "#94a3b8");
      next.na_bg = cssColor(rules, "--rc-na-bg", base.na_bg || "#f8fafc");
      next.na_color = cssColor(rules, "--rc-na-color", base.na_color || "#94a3b8");
      next.row_font_size = cssNumber(rules, "--rc-row-font-size", base.row_font_size || 8.5);
      next.na_logo_opacity = clamp(cssNumber(rules, "--rc-na-logo-opacity", base.na_logo_opacity ?? 0.07), 0, 1);
    } else if (type === "chart") {
      ...
      next.title_color = cssColor(rules, "--rc-title-color", base.title_color || "#164e63");
      next.grid_color = cssColor(rules, "--rc-grid-color", base.grid_color || "#e2e8f0");
      next.axis_color = cssColor(rules, "--rc-axis-color", base.axis_color || "#64748b");
    }
    next.w_pct = clamp(next.w_pct, 1, 100 - next.x_pct);
    next.h_pct = clamp(next.h_pct, 1, 100 - next.y_pct);
    return next;
  });
  return { layout_json: layout, orientation };
};
...
export default function ReportCardVisualDesigner() {
  ...
  const hydrate = (template) => {
    ...
    setDraft(nextDraft);
    setCodeHtml(designToHtml(nextDraft));
    setCodeCss(designToCss(nextDraft));
    setCodeDirty(false);
  ...
  const syncCodeFromDesign = () => {
    if (!draft) return;
    setCodeHtml(designToHtml(draft));
    setCodeCss(designToCss(draft));
    setCodeDirty(false);
  };

  const parseCodeDraft = () => {
    const parsed = codeToDesign(codeHtml, codeCss, draft);
    return { ...draft, ...parsed };
  };
  ...
  const applyCodeToDesign = ({ silent = false } = {}) => {
    try {
      ...
      if (!silent) Swal.fire({ icon: "success", title: "Code applied", text: "HTML/CSS changes are now reflected on the visual canvas.", timer: 1200, showConfirmButton: false });
      return nextDraft;
    } catch (error) {
      Swal.fire("Could not apply code", error.message || "Please check the HTML/CSS and try again.", "error");
      return null;
    }
  };
  ...
  const changeViewMode = async (nextMode) => {
    ...
    if (viewMode === "code" && codeDirty) {
      const result = await Swal.fire({
        title: "Apply HTML/CSS changes?",
        text: "You have code changes that are not yet on the visual canvas.",
        icon: "question",
        showCancelButton: true,
        showDenyButton: true,
        confirmButtonText: "Apply changes",
        denyButtonText: "Discard code changes",
        cancelButtonText: "Stay here",
      });
      if (result.isDismissed) return;
      if (result.isConfirmed && !applyCodeToDesign({ silent: true })) return;
      if (result.isDenied) syncCodeFromDesign();
    }
    if (nextMode === "code") {
      setCodeHtml(designToHtml(draft));
      setCodeCss(designToCss(draft));
  ...
  const copyCode = async (kind) => {
    const value = kind === "css" ? codeCss : codeHtml;
    try {
      await navigator.clipboard.writeText(value);
      Swal.fire({ icon: "success", title: `${kind.toUpperCase()} copied`, timer: 700, showConfirmButton: false });
    } catch (_) {
      Swal.fire("Copy failed", "Please select the code and copy it manually.", "warning");
    }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await api.put(`/report-card/templates/${workingDraft.id}`, {
        ...
        is_active: workingDraft.is_active !== false, notes: workingDraft.notes || null, layout_json: workingDraft.layout_json || [],
      });
    ...
    <div className="rcvb-page">
      ...
      {draft ? <div className="rcvb-modebar">
        <div className="rcvb-view-tabs" role="tablist" aria-label="Report card editor mode">
          ...
          <button type="button" className={viewMode === "code" ? "active" : ""} onClick={() => changeViewMode("code")}><i className="bi bi-code-slash me-1" />HTML / CSS{codeDirty ? <span className="rcvb-dirty-dot" title="Unapplied code changes" /> : null}</button>
        ...
        <div className="rcvb-mode-hint">{viewMode === "code" ? "Two-way editor: edit supported HTML/CSS, then Apply to Design." : viewMode === "preview" ? "Clean live preview with sample ERP data." : "Drag, resize and configure smart ERP widgets."}</div>
      </div> : null}
      ...
      {!draft ? <div className="rcvb-empty"><i className="bi bi-layout-text-window-reverse" /><h4>Create your first visual report card</h4><p>Use a blank A4 canvas or add the school's existing PDF/image as a background.</p><Button onClick={() => setShowCreate(true)}>Create Template</Button></div> : viewMode === "code" ? (
        <div className="rcvb-code-workspace">
          <div className="rcvb-code-header">
            <div><strong>HTML / CSS Editor</strong><span>Smart components stay connected to ERP data. Scripts and JavaScript are blocked.</span></div>
            <div className="d-flex gap-2">
              <Button size="sm" variant="outline-secondary" onClick={syncCodeFromDesign}><i className="bi bi-arrow-counterclockwise me-1" />Reset from Visual</Button>
              <Button size="sm" onClick={() => applyCodeToDesign()} disabled={!codeDirty}><i className="bi bi-check2-circle me-1" />Apply to Design</Button>
            </div>
          </div>
          <div className="rcvb-code-grid">
            <section className="rcvb-code-panel">
              <div className="rcvb-code-panel-title"><div><i className="bi bi-filetype-html" /><strong>HTML Structure</strong></div><Button size="sm" variant="outline-light" onClick={() => copyCode("html")}><i className="bi bi-copy" /></Button></div>
              <textarea aria-label="Report card HTML" spellCheck="false" value={codeHtml} onChange={(e) => { setCodeHtml(e.target.value); setCodeDirty(true); }} />
            </section>
            <section className="rcvb-code-panel">
              <div className="rcvb-code-panel-title"><div><i className="bi bi-filetype-css" /><strong>CSS Layout & Style</strong></div><Button size="sm" variant="outline-light" onClick={() => copyCode("css")}><i className="bi bi-copy" /></Button></div>
              <textarea aria-label="Report card CSS" spellCheck="false" value={codeCss} onChange={(e) => { setCodeCss(e.target.value); setCodeDirty(true); }} />
            </section>
          </div>
          <div className="rcvb-code-footer">
            <div><i className="bi bi-shield-check" /><span>Safe mode reads layout/style properties only. Keep <code>data-rc-element="true"</code>; use <code>&lt;smart-marks-table&gt;</code> and <code>&lt;subject-growth-chart&gt;</code> for live ERP widgets.</span></div>
            <div className={codeDirty ? "rcvb-code-status dirty" : "rcvb-code-status"}>{codeDirty ? "Changes waiting to be applied" : "Code matches visual design"}</div>
          </div>
        </div>
      ) : viewMode === "preview" ? (
        <div className="rcvb-preview-workspace">
          <div className="rcvb-preview-toolbar">
            <div><strong>Live Preview</strong><span>Sample ERP values • saved PDF uses live student data</span></div>
          ...
          <div className="rcvb-preview-scroll">
            <div className="rcvb-canvas-shell" style={{ width: `${zoom}%` }}>
              <div className={`rcvb-canvas rcvb-preview-canvas ${draft.orientation === "landscape" ? "landscape" : "portrait"}`}>
                {draft.background_file_url ? (String(draft.background_mime || "").includes("pdf") || /\.pdf(?:$|\?)/i.test(draft.background_file_url) ? <iframe title="Report card background preview" src={`${draft.background_file_url}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`} className="rcvb-background" /> : <img src={draft.background_file_url} alt="Report card background" className="rcvb-background" />) : null}
                {(draft.layout_json || []).filter((el) => Number(el.page || 1) === Number(activePage)).map((el) => <div key={el.id} className={`rcvb-preview-element type-${el.type}`} style={{ left: `${el.x_pct}%`, top: `${el.y_pct}%`, width: `${el.w_pct}%`, height: `${el.h_pct}%`, zIndex: 10, opacity: Number(el.opacity ?? 1) }}>{renderElement(el)}</div>)}
              </div>
            </div>
          </div>
        </div>
