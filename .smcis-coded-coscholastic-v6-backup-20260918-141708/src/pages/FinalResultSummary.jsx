...
const FinalResultSummary = () => {
 ...
 const getSubjectTermCompDisplay = (student, subjectName, termId, componentId) => {
  ...
  const buildSmartTemplateData = (student, infoOverride = {}) => {
    ...
    // SMCIS_DYNAMIC_ONLY_CO_SCHOLASTIC_V5
    // IMPORTANT: this block is intentionally limited to the coded SMCIS
    // Dynamic template. Other report-card templates keep their existing path.
    const smcisDynamicCoScholasticRowsV5 = (() => {
      ...
      const putRow = (name, grade = "-", order = 0, source = "area") => {
        const cleanName = String(name || "").trim();
        if (!cleanName) return;
        const key = keyOf(cleanName);
        const next = {
          name: cleanName,
          grade: validGrade(grade) ? String(grade).trim() : "-",
          serial_order: Number(order || 0),
          source,
        };
        const old = rowsMap.get(key);
        if (!old) {
          rowsMap.set(key, next);
          return;
        }
        if (!validGrade(old.grade) && validGrade(next.grade)) old.grade = next.grade;
        if (Number(next.serial_order) < Number(old.serial_order || 0)) old.serial_order = next.serial_order;
      };

      // 1) Existing Co-Scholastic Area module rows (Drawing etc.).
      const displayTermId = term2Id || term1Id || null;
      if (displayTermId) {
        const bucket = coScholasticByTerm[String(displayTermId)] || {};
        const areaRows = Object.values(bucket[String(student.id)] || {});
        areaRows.forEach((row, index) => {
          putRow(
            row?.area_name || row?.name || `Activity ${index + 1}`,
      ...
      // 2) Subjects explicitly marked Co-Scholastic in Subject Master.
      // This is the source needed for General Knowledge / Value Education etc.
      (subjects || []).forEach((subject, index) => {
        const type = String(subject?.type || "").trim().toLowerCase();
        if (type !== "co-scholastic" && type !== "coscholastic") return;

        const subjectName = String(subject?.name || "").trim();
        if (!subjectName) return;
        const subjectId = subject?.id;
        const subjectKey = keyOf(subjectName);
        const componentRows = (student?.components || []).filter((component) => {
          const byId = subjectId != null && String(component?.subject_id ?? component?.subjectId ?? "") === String(subjectId);
          const byName = keyOf(component?.subject_name || component?.subject) === subjectKey;
          return byId || byName;
        });

        const directGrade = pickGrade(componentRows);
        let termGrade = "-";
        if (displayTermId) {
          termGrade = getSubjectTermStats(student, subjectName, displayTermId)?.grade || "-";
        }
        const grade = validGrade(directGrade) ? directGrade : termGrade;

        // Always include the row. If grade entry is pending, show '-'.
        putRow(subjectName, grade, 10000 + index, "subject");
      });

      // 3) Old SMCIS data sometimes has Drawing only through the legacy helper.
      const drawingGrade = term2Id
        ? [drawingT2, drawingT1].find(validGrade)
        : [drawingT1, drawingT2].find(validGrade);
      if (validGrade(drawingGrade)) putRow("Drawing", drawingGrade, 9000, "drawing");
    ...
    return {
      ...
      co_scholastic: selectedReportTemplate?.template_key === "smcis_dynamic_term_report_card_v1"
        ? { rows: smcisDynamicCoScholasticRowsV5 }
