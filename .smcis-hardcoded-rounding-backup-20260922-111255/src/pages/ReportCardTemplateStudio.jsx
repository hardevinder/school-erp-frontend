...
const ACCEPTED_DESIGN_MIME_TYPES = new Set([
  "application/pdf",
  "image/png",
  "image/jpeg",
  "image/jpg",
...
const ReportCardTemplateStudio = () => {
  ...
  const hydrateDraft = (template) => {
    ...
    const layoutContainer =
      !isInlineHtmlLayout && rawLayout && !Array.isArray(rawLayout) && typeof rawLayout === "object"
        ...
        : null;
    const printSettings =
      template.print_settings_json && typeof template.print_settings_json === "object"
        ? template.print_settings_json
        : {};
    ...
    setDraft({
      ...
      print_settings_json: {
        ...printSettings,
        grand_total_rounding: printSettings.grand_total_rounding || "none",
      },
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      Swal.fire("Created", "Template saved. Now drag & drop the school's PDF/image design.", "success");
    } catch (error) {
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      const res = await api.put(`/report-card/templates/${draft.id}`, {
        ...
        print_settings_json: draft.print_settings_json || { grand_total_rounding: "none" },
      });
      const item = res.data?.template;
      if (item) {
  ...
  const saveBackgroundFile = async (file) => {
    if (!file || !draft?.id || uploading) return;

    const fileName = String(file.name || "").toLowerCase();
    const hasAllowedExtension = /\.(pdf|png|jpe?g)$/i.test(fileName);
    const hasAllowedMime = ACCEPTED_DESIGN_MIME_TYPES.has(String(file.type || "").toLowerCase());

    if (!hasAllowedExtension && !hasAllowedMime) {
      Swal.fire("Unsupported file", "Please drop a PDF, PNG, JPG or JPEG report-card design.", "warning");
      return;
    }
    if (Number(file.size || 0) > MAX_DESIGN_FILE_SIZE) {
      Swal.fire("File too large", "Report-card design must be 25 MB or smaller.", "warning");
      return;
    }
    ...
    try {
      ...
      Swal.fire({
        title: "Design saved",
        text: "Report-card design is ready. You can run AI Auto Detect now.",
        icon: "success",
        timer: 1500,
        showConfirmButton: false,
      });
    } catch (error) {
      Swal.fire("Design import failed", error.response?.data?.message || "Could not save report-card design", "error");
    } finally {
      setUploading(false);
  ...
  const handleDesignFileInput = async (event) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    await saveBackgroundFile(file);
  };

  const handleDesignDragEnter = (event) => {
    event.preventDefault();
    event.stopPropagation();
    if (Array.from(event.dataTransfer?.types || []).includes("Files")) setDesignDragActive(true);
  };

  const handleDesignDragOver = (event) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  const handleDesignDragLeave = (event) => {
    event.preventDefault();
    event.stopPropagation();
    if (!event.currentTarget.contains(event.relatedTarget)) setDesignDragActive(false);
  };

  const handleDesignDrop = async (event) => {
    event.preventDefault();
    event.stopPropagation();
    setDesignDragActive(false);
    const files = Array.from(event.dataTransfer?.files || []);
    if (!files.length) return;
    await saveBackgroundFile(files[0]);
  };

  const analyzeWithAI = async () => {
    if (!draft?.background_file_url) {
      Swal.fire("Add design first", "Drag & drop the school's PDF/JPG/PNG report card before AI analysis.", "info");
      return;
    }
  ...
  const handleCanvasDrop = async (event) => {
    ...
    if (event.dataTransfer?.files?.length) {
      setDesignDragActive(false);
      await saveBackgroundFile(event.dataTransfer.files[0]);
      return;
    }
    ...
    <div className="rc-studio-page">
      <div className="rc-studio-hero">
        <div>
          ...
          <p>
            Drag & drop each school's existing PDF/image design, auto-detect dynamic fields with AI,
            ...
            {visibleTemplates.length === 0 ? (
              ...
              >
                ...
                <div className="mt-2 d-flex gap-1 flex-wrap">
                  {item.background_file_url ? <Badge bg="primary">Design added</Badge> : <Badge bg="secondary">No design</Badge>}
          ...
          {!draft ? (
            ...
            <>
              <div className="rc-toolbar-card">
                <div className="rc-template-title-row">
                  <div>
                    ...
                    <div className="text-muted small mt-1">
                      Version {draft.template_version || 1} • {draft.orientation || "portrait"} • {draft.source_file_name || "No source design added"}
                    </div>
                  </div>
                ...
                <div
                  className={`rc-design-dropzone mt-3 ${designDragActive ? "drag-active" : ""} ${uploading ? "busy" : ""}`}
                  onDragEnter={handleDesignDragEnter}
                  onDragOver={handleDesignDragOver}
                  onDragLeave={handleDesignDragLeave}
                  onDrop={handleDesignDrop}
                  onClick={() => !uploading && designFileInputRef.current?.click()}
                  onKeyDown={(event) => {
                ...
                >
                  <input
                    ...
                    disabled={uploading}
                  />
                  <div className="rc-design-drop-icon">
                    {uploading ? <Spinner animation="border" size="sm" /> : <i className="bi bi-cloud-arrow-up" />}
                  </div>
                  <div className="rc-design-drop-copy">
                    <strong>
                      {uploading
                        ? "Saving report-card design…"
                        : draft.background_file_url
                        ? "Drag & drop a new design here to replace the current one"
                        : "Drag & drop the school's report-card design here"}
                    </strong>
                    <span>PDF, PNG or JPG • maximum 25 MB • click here to browse if needed</span>
                    {draft.source_file_name ? <small>Current design: {draft.source_file_name}</small> : null}
                  </div>
                  {!uploading && <span className="rc-design-drop-action">Drop file</span>}
                </div>
                ...
                <div className="border rounded-3 bg-light p-3 mt-3">
                  <div className="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                    <div>
                      <div className="fw-semibold">Report Card Print Settings</div>
                      <div className="small text-muted">
                        Template-specific display rules. Marks calculations remain unchanged.
                      </div>
                    </div>
                    <div style={{ minWidth: 320 }}>
                      <Form.Check
                        type="switch"
                        id={`rc-grand-total-rounding-${draft.id}`}
                        label="Round final Grand Total to whole number"
                        checked={
                          String(draft.print_settings_json?.grand_total_rounding || "none") ===
                          "nearest_integer"
                        }
                        onChange={(e) =>
                          setDraft((prev) => ({
                            ...prev,
                            print_settings_json: {
                              ...(prev?.print_settings_json || {}),
                              grand_total_rounding: e.target.checked ? "nearest_integer" : "none",
                            },
                          }))
                        }
                      />
                      <div className="form-text mt-1">
                        Grand Total only: decimal below 0.50 rounds down; 0.50 or above rounds up.
                        Subject totals, term totals, percentage and grade are not changed.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <section className="rc-design-area">
                  <div className="rc-design-head">
                    <div>
                      ...
                      <div className="small text-muted">Blue boxes are dynamic ERP values. The source design remains untouched.</div>
                    </div>
                  ...
                  <div
                    ref={canvasRef}
                    className={`rc-design-canvas ${draft.orientation === "landscape" ? "landscape" : "portrait"} ${designDragActive ? "file-drag-active" : ""}`}
                    onDragEnter={(e) => {
                      if (e.dataTransfer?.types?.includes("Files")) setDesignDragActive(true);
                    }}
                    onDragOver={(e) => {
                      e.preventDefault();
                    ...
                    onDragLeave={(e) => {
                      if (!e.currentTarget.contains(e.relatedTarget)) setDesignDragActive(false);
                    }}
                  ...
                  >
                    {draft.background_file_url ? (
                      ...
                      <div className="rc-no-background">
                        ...
                        <strong>Drag & drop the school's existing report card</strong>
                        <span>Drop it here or use the drop zone above • PDF, PNG or JPG</span>
                      </div>
                    )}
              ...
              <div className="rc-studio-tip mt-3">
                ...
                <div>
                  <strong>Recommended workflow:</strong> Create Primary / Middle / Senior templates → drag & drop each approved school design → run AI Auto Detect → visually adjust blue fields once → save. The same template is reused automatically for every student in the assigned classes.
                </div>
              </div>
            </>
          )}
